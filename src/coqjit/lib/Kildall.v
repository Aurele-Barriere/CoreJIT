...
Module Type DATAFLOW_SOLVER.
  ...
  Local Parameter fixpoint_solution:
  ...
  Local Parameter fixpoint_entry:
  ...
  Local Parameter fixpoint_invariant:
...
Module Type NODE_SET.
  ...
  Local Parameter add: positive -> t -> t.
  Local Parameter pick: t -> option (positive * t).
  Local Parameter initial: PTree.t (list positive) -> t.

  Local Parameter In: positive -> t -> Prop.
  Local Parameter add_spec:
    forall n n' s, In n' (add n s) <-> n = n' \/ In n' s.
  Local Parameter pick_none:
    forall s n, pick s = None -> ~In n s.
  Local Parameter pick_some:
    forall s n s', pick s = Some(n, s') ->
    forall n', In n' s <-> n = n' \/ In n' s'.
  Local Parameter initial_spec:
...
Module Type ORDERED_TYPE_WITH_TOP.
  ...
  Local Parameter t: Type.
  Local Parameter ge: t -> t -> Prop.
  Local Parameter top: t.
  Local Parameter top_ge: forall x, ge top x.
  Local Parameter refl_ge: forall x, ge x x.
